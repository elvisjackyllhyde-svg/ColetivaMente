...
export async function sendAccountEmail(to: string, subject: string, html: string) {
  ...
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json", "user-agent": "ColetivaMente/1.0" }, body: JSON.stringify({ from, to: [to], subject, html }) });
