...
export type User = { id: number; name: string; email: string; company: string; subscriptionStatus: string; subscriptionExpiresAt: string | null; isAdmin: boolean; freeTrialsRemaining: number };
...
export function AccountPage() {
  ...
  return <><AuthHeader /><main className="aCenter accountCenter"><div className="accountLayout"><section className="aCard">
    ...
    {!user.isAdmin && !active && <div className="aStatusRow"><span>TESTES GRÁTIS</span><b className={user.freeTrialsRemaining > 0 ? "aActive" : "aInactive"}>{user.freeTrialsRemaining} de 2 restantes</b></div>}
...
export function RequireActiveSubscription({ children }: { children: React.ReactNode }) {
  ...
  if (!hasCreationAccess(user)) return <><AuthHeader /><main className="aCenter"><section className="aCard"><p className="aEyebrow">ASSINATURA NECESSÁRIA</p><h1>Assine para criar dinâmicas</h1><p className="aMuted">Seus 2 testes gratuitos terminaram. Assine por R$67/mês ou R$564 à vista por 1 ano (R$47/mês).</p><a className="aBtn primary big" href="/?modo=conta">Ir para minha conta →</a></section></main></>;
  return <>{children}</>;
}"use client";
import { useEffect, useState } from "react";
import "./auth.css";
