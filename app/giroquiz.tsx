...
export function GiroQuizApp() {
  ...
  if(screen==="setup")return <div className="gq"><SetupScreen title={quizTitle} subject={quizSubject} questions={customQuestions} savedQuizzes={savedQuizzes} niche={studyNiche} categoryTopic={categoryTopic} count={studyCount} difficulty={difficulty} pdfName={pdfName} pdfLoading={pdfLoading} loading={loading} error={error} musicTrack={musicTrack} musicScope={musicScope} onTitle={setQuizTitle} onSubject={setQuizSubject} onQuestions={setCustomQuestions} onManualStart={startManualQuestions} onCount={n=>{setStudyCount(n);if(studyNiche)setCustomQuestions(makeStudyQuiz(studyNiche,n,difficulty));else if(categoryTopic)setCustomQuestions(makeCategoryQuiz(categoryTopic,n,difficulty));}} onDifficulty={d=>{setDifficulty(d);if(studyNiche){setCustomQuestions(makeStudyQuiz(studyNiche,studyCount,d));setQuizSubject(`${nicheInfo[studyNiche].description} · nível ${d}`);}else if(categoryTopic){setCustomQuestions(makeCategoryQuiz(categoryTopic,studyCount,d));setQuizSubject(`${categoryNames[categoryTopic]} · nível ${d}`);}}} onNiche={useStudyNiche} onCategory={useCategoryTopic} onPdf={importPdf} onOriginal={()=>{setStudyNiche("");setCategoryTopic("");setCustomQuestions(null);setPdfName("");setQuizTitle("GiroQuiz");setQuizSubject("Desafio de conhecimento");}} onOpenSaved={openSavedQuiz} onDeleteSaved={deleteSavedQuiz} onMusicTrack={setMusicTrackLocal} onMusicScope={setMusicScopeLocal} onBack={()=>setScreen("home")} onCreate={createRoom}/></div>;
...
function SetupScreen({title,subject,questions,savedQuizzes,niche,categoryTopic,count,difficulty,pdfName,pdfLoading,loading,error,musicTrack,musicScope,onTitle,onSubject,onQuestions,onManualStart,onCount,onDifficulty,onNiche,onCategory,onPdf,onOriginal,onOpenSaved,onDeleteSaved,onMusicTrack,onMusicScope,onBack,onCreate}:{title:string;subject:string;questions:CustomQuestion[]|null;savedQuizzes:SavedQuiz[];niche:StudyNiche|"";categoryTopic:CategoryTopic|"";count:number;difficulty:Difficulty;pdfName:string;pdfLoading:boolean;loading:boolean;error:string;musicTrack:string;musicScope:MusicScope;onTitle:(v:string)=>void;onSubject:(v:string)=>void;onQuestions:(v:CustomQuestion[]|null)=>void;onManualStart:()=>void;onCount:(v:number)=>void;onDifficulty:(v:Difficulty)=>void;onNiche:(v:StudyNiche)=>void;onCategory:(v:CategoryTopic)=>void;onPdf:(f?:File)=>void;onOriginal:()=>void;onOpenSaved:(quiz:SavedQuiz)=>void;onDeleteSaved:(id:number)=>void;onMusicTrack:(id:string)=>void;onMusicScope:(s:MusicScope)=>void;onBack:()=>void;onCreate:()=>void}){
 ...
 {!niche&&!categoryTopic&&questions&&<ManualQuestionEditor questions={questions} onQuestions={onQuestions}/>} 
