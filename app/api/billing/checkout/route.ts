...
import { mpSecrets, PLANS, PRODUCTION_ORIGIN, webhookSecretConfigured, type PlanId } from "../../../../lib/mercado-pago";
...
export async function POST(request:Request){
 ...
 if(!webhookSecretConfigured())return Response.json({error:"A assinatura secreta do webhook é obrigatória antes de receber pagamentos."},{status:503});
 const body=await request.json().catch(()=>({})) as {plan?:string};
 const planId:PlanId=body.plan==="annual"?"annual":"monthly";
 const plan=PLANS[planId];
 const reference=crypto.randomUUID(),origin=PRODUCTION_ORIGIN;
 await db.insert(payments).values({userId:user.id,externalReference:reference,amountCents:plan.amountCents,accessDays:plan.accessDays,currency:"BRL"});
 await writeAuditEvent({request,category:"payment",action:"checkout_created",actorUserId:user.id,targetUserId:user.id,resourceType:"payment",resourceId:reference,metadata:{amountCents:plan.amountCents,accessDays:plan.accessDays,currency:"BRL",provider:"mercado_pago"}});
 const response=await fetch("https://api.mercadopago.com/checkout/preferences",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json","X-Idempotency-Key":reference},body:JSON.stringify({items:[{id:`coletivamente-${planId}`,title:plan.title,description:"GiroQuiz, Voto em Valor e Sorteio",quantity:1,currency_id:"BRL",unit_price:plan.amountCents/100}],external_reference:reference,notification_url:`${origin}/api/billing/webhook`,back_urls:{success:`${origin}/?modo=conta&pagamento=success`,pending:`${origin}/?modo=conta&pagamento=pending`,failure:`${origin}/?modo=conta&pagamento=failure`},auto_return:"approved",statement_descriptor:"COLETIVAMENTE"})});
