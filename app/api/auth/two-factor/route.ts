...
export async function POST(request: Request) {
  ...
  if (!token) return Response.json({ error: "Código inválido ou expirado." }, { status: 401 });
  ...
  if (!(await verifyTotp(token.secret, String(code)))) return Response.json({ error: "Código inválido ou expirado." }, { status: 401 });
