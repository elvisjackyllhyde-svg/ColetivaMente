...
export async function POST(req: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (!room || room.host_key !== hostKey || !user || (!user.isAdmin && room.creator_user_id !== null && room.creator_user_id !== user.id)) return Response.json({ error: "Acesso administrativo inválido" }, { status: 403 });
