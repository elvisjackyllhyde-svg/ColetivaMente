...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    const { slug } = await params; const body = await request.json() as { selections?: number[]; voterId?: string; name?: string; email?: string }; const db = getDb();
    ...
    await db.insert(participants).values({ campaignId: campaign.id, voterId: body.voterId.slice(0, 80), name, email });
