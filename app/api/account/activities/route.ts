...
export async function GET(request: Request) {
  ...
  const [quizResult, campaignResult, raffleResult] = await Promise.all([
    d1.prepare(`SELECT r.code id, COALESCE(q.title,'GiroQuiz') title, COALESCE(q.subject,'') subtitle, r.status status, r.created_at created_at,
  ...
  const activities = [
    ...quizResult.results.map(item => ({ ...item, type: "quiz", createdAt: normalizeDate(item.created_at), url: `/?modo=quiz&gerenciar=${encodeURIComponent(item.id)}` })),
