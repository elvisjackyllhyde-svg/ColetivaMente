...
export function RaffleView({ slug, admin }: { slug: string; admin: string }) {
  ...
  const drawNumber = (drawId: string) => `SRT-${drawId.replace(/-/g, "").slice(0, 8).toUpperCase()}`;
  const shareRound = async (round: AccountHistoryWinner[]) => {
    const ordered = [...round].sort((a, b) => a.position - b.position);
    const raw = ordered[0].drawnAt;
    const date = new Date(raw.includes("T") ? raw : `${raw.replace(" ", "T")}Z`);
    const text = [`🎉 RESULTADO DO SORTEIO`, ordered[0].raffleTitle, `Número: ${drawNumber(ordered[0].drawId)}`, `Data: ${date.toLocaleDateString("pt-BR")} às ${date.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}`, "", ...ordered.map((winner, index) => `${index + 1}º — ${winner.name}`), "", "Sorteio realizado pela ColetivaMente"].join("\n");
    try {
      if (navigator.share) await navigator.share({ title: `Resultado ${drawNumber(ordered[0].drawId)}`, text });
